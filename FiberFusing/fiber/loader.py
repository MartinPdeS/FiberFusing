...
def load_fiber(fiber_name: str, clad_refractive_index: float, position: Tuple[float, float] = (0, 0), remove_cladding: bool = False) -> GenericFiber:
    ...
    position : Tuple[float, float], optional
        The spatial position of the fiber, defaulting to (0, 0).
    remove_cladding : bool, optional
        Whether to exclude the cladding layer from the fiber configuration.
    ...
    GenericFiber
        A configured GenericFiber object with the specified properties.
    """
    return _default_loader.load_fiber(fiber_name, clad_refractive_index, position, remove_cladding)
...
def make_fiber(structure_list: List[Dict], clad_refractive_index: float, position: Tuple[float, float] = (0, 0)) -> GenericFiber:
    ...
    clad_refractive_index : float
        The refractive index of the cladding material.
    position : Tuple[float, float], optional
        The spatial position of the fiber.
    ...
    GenericFiber
        The newly created GenericFiber object populated with specified structures.
    """
    return _default_loader.make_fiber(structure_list, clad_refractive_index, position)
...
def load_fiber_as_dict(fiber_name: str, clad_refractive_index: float = None, order: str = 'in-to-out') -> dict:
    ...
    clad_refractive_index : float, optional
        Refractive index to use for material index calculation.
    order : str, optional
        Layer order in the output dictionary; 'in-to-out' or 'out-to-in' (for backward compatibility).
